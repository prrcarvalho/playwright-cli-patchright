...
1. **Sync upstream Microsoft `playwright-cli` first (if needed):**
   ```bash
   # One-time setup
   ...
   ```
   Then continue with Patchright runtime roll.

2. **Run the automatic upgrade command**:
   ```bash
...
3. **If you need a manual roll instead, check latest Patchright version**:
   ```bash
   npm info patchright version
   ```

4. **Update Patchright packages** in `package.json`:
   - Update `patchright-core` (dependency) and `patchright` (devDependency) to the target version.
   - Run `npm install` to update `package-lock.json`.

5. **Run the update script** to sync the bundled patched skill into the generated skills location:
   ```bash
...
6. **Check the command surface** against the pinned Patchright runtime:
   ```bash
...
7. **Update README.md and skills** with relevant command changes. Compare the command list with `skills/playwright-cli-patched/SKILL.md` and `README.md`, then update sections that are out of date (commands, flags, default behaviours, examples, Patchright caveats).

8. **Verify** the CLI works:
   ```bash
