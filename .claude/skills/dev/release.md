...
4. **Bump this wrapper version** in `package.json`, then run `npm install` to sync `package-lock.json`.

5. **Verify the runtime wiring.**
   ```bash
...
7. **Write release notes** in `RELEASE_NOTES_v<version>.md`.
   Include:
   - wrapper version
...
8. **Commit and open a PR against the fork.**
   ```bash
